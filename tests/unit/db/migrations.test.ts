...
import {
  allMigrationVersions,
  migrationVersionsAbove,
...
describe("runMigrations", () => {
  it("creates the latest schema with every table on a fresh DB", () => {
    ...
    expect(r.applied).toEqual(allMigrationVersions());
  ...
  it("creates v32 review_refute_votes and v33 phase review_state_version sequentially", () => {
    ...
    try {
      ...
      expect(upgraded.applied).toEqual(migrationVersionsAbove(31));
  ...
  it("rebuilds hitch_lifecycle_events in v29 without loosening constraints", () => {
    ...
    try {
      ...
      const upgraded = runMigrations(db);
      expect(upgraded.applied).toEqual(migrationVersionsAbove(28));
      expect(upgraded.version).toBe(SCHEMA_VERSION);
      ...
      // #280 (v34): the MIGRATED table's rebuilt CHECK must ACCEPT
      // `diverging_recovered` too (positive assertion on the upgraded path, not
      // just the fresh-DB path) — guards a future rebuild that copies rows but
      // omits the new value from the migrated CHECK list.
      for (const event of [
        "reopened",
        "closed",
        "cancelled",
  ...
  it("creates domain_lock_contention in v28 and stays idempotent", () => {
    ...
    try {
      ...
      const upgraded = runMigrations(db);
      expect(upgraded.applied).toEqual(migrationVersionsAbove(27));
      expect(upgraded.version).toBe(SCHEMA_VERSION);
  ...
  it("recreates run_usage in v30 with per-invocation primary key and preserves v26 rows", () => {
    ...
    try {
      ...
      const upgraded = runMigrations(db);
      expect(upgraded.applied).toEqual(migrationVersionsAbove(29));
      expect(upgraded.version).toBe(SCHEMA_VERSION);
  ...
  it("creates metrics_snapshots in v27 and stays idempotent", () => {
    ...
    try {
      ...
      const upgraded = runMigrations(db);
      expect(upgraded.applied).toEqual(migrationVersionsAbove(26));
      expect(upgraded.version).toBe(SCHEMA_VERSION);
  ...
  it("creates run_usage from v26 and upgrades it to the latest per-invocation shape", () => {
    ...
    try {
      ...
      const upgraded = runMigrations(db);
      expect(upgraded.applied).toEqual(migrationVersionsAbove(25));
      expect(upgraded.version).toBe(SCHEMA_VERSION);
  ...
  it("adds run execution environment provenance columns in v25 and stays idempotent", () => {
    ...
    try {
      ...
      const upgraded = runMigrations(db);
      expect(upgraded.applied).toEqual(migrationVersionsAbove(24));
      expect(upgraded.version).toBe(SCHEMA_VERSION);
      const after = db
        .prepare("PRAGMA table_info(runs)")
  ...
  it("adds review_proposals.prompt_provenance_json in v24 and stays idempotent", () => {
    ...
    try {
      ...
      const upgraded = runMigrations(db);
      expect(upgraded.applied).toEqual(migrationVersionsAbove(23));
      expect(upgraded.version).toBe(SCHEMA_VERSION);
      const after = db
        .prepare("PRAGMA table_info(review_proposals)")
        .all() as { name: string; type: string; notnull: number }[];
  ...
  it("creates hitch_lifecycle_events in v23 and stays idempotent", () => {
    ...
    try {
      ...
      const upgraded = runMigrations(db);
      expect(upgraded.applied).toEqual(migrationVersionsAbove(22));
      expect(upgraded.version).toBe(SCHEMA_VERSION);
  ...
  it("drops db_stats_snapshots in v22 and stays idempotent", () => {
    ...
    try {
      ...
      const upgraded = runMigrations(db);
      expect(upgraded.applied).toEqual(migrationVersionsAbove(21));
      expect(upgraded.version).toBe(SCHEMA_VERSION);
...
describe("v35 artifact quarantine marker backfill (#303 P1)", () => {
  ...
  it("grandfathers legacy absent recoverable rows so the next full sync does not prune them", () => {
    ...
    // Upgrade through v35.
    const upgraded = runMigrations(db);
    expect(upgraded.applied).toEqual(migrationVersionsAbove(34));
