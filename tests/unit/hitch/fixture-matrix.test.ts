...
class SimulatedGoalLoop {
  readonly decisions: HitchConvergenceResult[] = [];
  readonly statusHistory: HitchStatus[] = [];

  constructor(private readonly ctx: FixtureContext) {}

  evaluate(): HitchConvergenceResult {
    const result = this.ctx.service.evaluate(this.ctx.hitchId);
    this.ctx.repo.recordConvergenceDecision({
      hitchId: this.ctx.hitchId,
    ...
    this.statusHistory.push(this.ctx.repo.requireSession(this.ctx.hitchId).status);
    return result;
  }

  runUntilStop(input: {
    maxTurns?: number;
    shouldStop?: (result: HitchConvergenceResult) => boolean;
    advance: (result: HitchConvergenceResult) => void;
  }): HitchConvergenceResult[] {
...
function fresh(hitchId: string, overrides: {
...
function statusForDecision(decision: HitchConvergenceDecision): HitchStatus | null {
...
function defaultStop(result: HitchConvergenceResult): boolean {
  return (
...
function startCycle(ctx: FixtureContext, reviewMode?: HitchReviewMode) {
  const nextCycle = ctx.repo.listReviewCycles(ctx.hitchId).length + 1;
...
function decisionNames(loop: SimulatedGoalLoop): HitchConvergenceDecision[] {
...
describe("goal convergence fixture matrix", () => {
  it("converges after a simulated fix and close-check pass", () => {
    ...
    try {
      ...
      let blocker: HitchFinding | null = null;
      ...
      expect(ctx.repo.listAttempts(ctx.hitchId)).toHaveLength(2);
      expect(ctx.repo.listDecisions(ctx.hitchId)).toHaveLength(3);
    } finally {
      ctx.db.close();
    }
  });
  ...
  it("stops a simulated loop when review findings diverge", () => {
    ...
    try {
      ...
      expect(ctx.repo.requireSession(ctx.hitchId).status).toBe("diverging");
    } finally {
      ctx.db.close();
    }
  });
  ...
  it("pauses on unknown scope until classification resolves the loop", () => {
    ...
    try {
      ...
      expect(ctx.repo.listAttempts(ctx.hitchId)).toHaveLength(0);
    } finally {
      ctx.db.close();
    }
  });
  ...
  it("stops immediately when the iteration budget is exhausted", () => {
    ...
    try {
      ...
      expect(ctx.repo.requireSession(ctx.hitchId).status).toBe(
        "budget_exhausted",
      );
      expect(ctx.repo.listAttempts(ctx.hitchId).map((a) => a.iteration)).toEqual([
        1,
        2,
      ]);
    } finally {
