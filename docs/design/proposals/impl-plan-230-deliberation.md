...
### 主要 P2（実装計画に確定織り込み）
- **P2-a JURY_BATCH_LIMIT（Task D1）**: jury 専用 cap 定数を `orchestrator-runners.ts` に新設（既定値を明記・`FINDING_BATCH_LIMIT=200` 以下。per-finding 4〜7 codex 呼び出しを踏まえ小さめ）。heuristic 確定は既存 while-drain 維持。**jury 対象は cap 件のみ処理し、cap 到達時は『more-pending を表す結果』を返して orchestrator が当該 invocation を halt する**（codex#252）。単に `resolved:true` を返すと orchestrator が同一 run 内で continue→needs_classification 再評価→再 classify を `maxSteps` まで繰り返し、per-cycle コスト上界が崩れるため、**`resolved:true` だけに頼らず再入 guard（`juryProcessedThisRun` / 専用 outcome）で同一 invocation 内の jury 再実行を止める**。残 unknown は次回 orchestrate invocation の convergence 再評価で処理。RED:「1 invocation で jury batch が cap 回で止まる（maxSteps まで複数 cap batch を実行しない）／残 unknown が次 invocation で処理」。
...
- [ ] **Step 1: RED** — `resolved:true ∧ severityAuditPacket≠null` のとき `recordConvergenceDecisionWithStatus({updateStatus:false, decision:<非blocking>, recommendedNextAction.decisionPacket})` で **hitch status 不変・packet 永続化**。**`decision` は `escalate` を使わない**（updateStatus:false でも course rollup は最新 decision の値で blocking 判定するため `escalate` だと linked phase を block する）。scope unanimous + severity diverged で hitch status も course/phase rollup も不変であることを assert。
- [ ] **Step 1b: RED — course rollup 非干渉（codex#252-P2）** — course rollup は最新 `hitch_convergence_decisions` の **`decision` 値**で blocking 判定（`escalate` は blocked-set）。advisory severity record の `decision` を **rollup の blocked-set 外の非 blocking 値**（実装前に convergence decision enum と rollup blocked-set を grep し確定。`escalate`/`needs_fix` 不可）にする。RED:「severity 乖離 packet 記録後も course/phase rollup 状態が不変（newest decision が advisory でも block しない）」。
- [ ] **Step 2-4:** RED→GREEN→typecheck
