...
- ReviewRuleRequirement additive: `reviewerIds?:string[]` / `maxReviewers?:number` が optional・後方互換（既存 rule は未指定で従来挙動、CC2）。YAML `reviewer_ids`/`max_reviewers` → camelCase に compile マッピング
...
- runRefuteAgent binding verification fail → errors list、inserted=[]、DB row 未書込
