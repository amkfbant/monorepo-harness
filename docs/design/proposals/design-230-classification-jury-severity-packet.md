...
- **永続化パス（WI-9c — codex PR#246 :156/:400 反映）**: scope jury が unanimous（`resolved:true`）でも `auditSeverity()` が `diverged`/`inconclusive` を返したら、**握り潰さず必ず `severityAudit` packet を永続化する明示パス**を通す。WI-9b（classify 失敗=`resolved:false` 時の packet 記録）とは別に、**resolved:true かつ severity 乖離のケースで `recordConvergenceDecisionWithStatus` を non-escalating（status は据え置き／advisory）で1回呼び、`recommendedNextAction.decisionPacket` に `severityAudit` を載せて記録**する（escalate を強制せずとも DB に残り operator/dashboard から見える）。`classify` runner の戻り値（§3.3 構造化型）に「scope 確定したが severity 乖離あり」を表す `severityAuditPacket?` を持たせ、orchestrator は resolved:true の continue 前にこの packet があれば記録する。受け入れ条件②/WI-9c のテストはこの非escalating 永続化を assert する。
