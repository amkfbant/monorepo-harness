...
かつ**自動検証 kind**（`command` / `finding_policy` / `review_consensus`）と
**外部証拠待ち kind**（`manual` / `artifact_exists` / `operation_status` / `db_doctor`〔runner 未実装〕
→ `ask_human` に routing）を区別する。自動ゲートを意図するなら前者を使う。合議は条件を*書く*が、
判定は決定論ゲートが*行う*。
