...
- `evidence add`: `--label` は必須。`--output` と `--output-file` は排他。
  ...
  **その condition id に紐づき、かつ FRESH（`created_at` ≥ `freshAfter`）で、condition の
...
`ReviewWorkflowUnsupportedError` で fail-closed 拒否する。これは `--dry-run` でも
同じで、consensus profile は resolved policy 表示へ進まない。`reviewed-run` は単一
...
3. **DB-backed ∧ export-OFF の場合は codex 起動直前に prior-output を INPUT-ALLOWLIST で一掃**（`quarantinePriorReviewerVerdictArtifacts`・#272）: `runs/<id>/` 直下で reviewer の input allowlist（`meta.json` / `events.jsonl` / `REVIEWER_INPUT_FILES` / `commands/`）だけを残し、それ以外の全エントリ（`reviewers/**` の reviewer/refute transcript、`review-evaluations/**`、root `review-decision.yaml`、将来の任意 producer 出力）を一掃する。ingestable なものは **まず DB に ingest（監査用に recoverable）→ 削除**、raw dotfile stream（`.reviewer-agent.events.raw.jsonl` 等）は ingest 不可ゆえ **REMOVE-ONLY**（non-recoverable・canonical な published `reviewer-agent.events.jsonl` のみ recover 可）。codex `--sandbox read-only` は絶対パスの read を jail しないため、denylist でなく allowlist 反転で現在/将来の verdict-bearing producer を取りこぼさない。verdict は `review_proposals`、ingestable transcript は DB artifact が canonical なのでディスクから消しても失われない（最終 export が DB から再生）。その OS-temp dir を cwd に、`sandbox=read-only` で codex を起動
...
7. `--dry-run` 未指定なら DB-backed run では `review_proposals` に proposal を記録する（**verdict は DB-canonical**）。per-reviewer の `reviewers/<reviewer_id>/review-decision.yaml` は **default（export OFF）では round 中ディスクに書かない**（#272：step 3 の root+scoped 掃除と合わせ、reviewer codex 実行中は verdict を一切ディスクに残さない）。`HARNESS_EXPORT_FILES=1` のときだけ互換 sidecar として書く。DB がない legacy run では root `review-decision.yaml` を canonical sidecar として常に書く。stale な scoped `review-auto-error.json` は削除する
...
- **round 中は先行の verdict をディスクに残さない（#272）**: DB-backed ∧ export-OFF では reviewer codex 起動直前に prior-output を **INPUT-ALLOWLIST 反転**で一掃する。reviewer の input allowlist（`meta.json` / `events.jsonl` / `REVIEWER_INPUT_FILES` / `commands/`）だけを残し、それ以外の全 producer 出力 — root/scoped `review-decision.yaml`、reviewer transcript（`reviewer-agent.out.log` / `.err.log` / events / `.redacted.tmp`）、**refute transcript（`refute-agent.*`）**、**`review-evaluations/**`**、将来の任意の producer、過去 export-ON / rerun の stale 含む — を一掃する（redaction は secret-only で verdict を落とさないため events 系も対象）。ingestable なものは DB ingest 後に削除、raw dotfile stream は ingest 不可ゆえ REMOVE-ONLY（non-recoverable）。verdict は `review_proposals`、ingestable transcript は DB artifact が canonical。codex `--sandbox read-only` は絶対パスの read を jail しない（codex-cli 0.139 に readable-root 設定無し）ため、ディスクに verdict を残さないことが harness 側で決定論的に強制できる唯一の独立性保証。recoverable な artifact は `HARNESS_EXPORT_FILES=1` / `harness db export-files` で DB から再生する。**verdict 以外**の repo / run ファイルの absolute-read は read-only codex で依然可能（verdict-leak class は本 fix で complete・full OS-level read-jail は best-effort で `docs/future-features.md` に defer）
...
| `update` | phase の declared status / scope/close conditions / 監査 note を更新。複数指定時は `PhaseRepository.update()` が 1 つの `BEGIN IMMEDIATE` transaction で適用し、scope/close・status・note のどれかが失敗したら全体を rollback する。scope/close conditions は hitch と同じ close-condition validator と `--allow-scope-widen` / `--allow-gate-loosen` gate を通る。`--note <text>` は force-close 理由や PR ref を記録し（`review_state_json` の `{ note }` に保存・migration 不要）、`course export --md` に `**Note**:` 行として出る（#171b） |
