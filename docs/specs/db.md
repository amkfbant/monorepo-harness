...
- **target binding**: `target_change_hash` は app 層の
  ...
  `sha256(normalizeChangeText(change_text))`。`normalizeChangeText` は NFC、
  `CRLF`/`CR`→`LF`、行内の tab/space 連続を単一 space、行末空白除去、
  全体 trim を行い、case folding / 句読点除去はしない。`target_change_idx`
  は記録時点の advisory index で、binding の正本ではない。
...
- `normalizeChangeText()` / `targetChangeHash()` /
  ...
  fail-closed で bind できる場合だけ集約入力にできる。
