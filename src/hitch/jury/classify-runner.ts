...
function runPhase3(
  ...
  return withManagedDb({ dbPath: deps.dbPath }, (db) => {
    ...
    if (heuristic.noProgressEscalate !== undefined) {
      // P2k: even when the heuristic drain escalates, persist any audit rows
      // already generated by the expensive Phase 2 deliberation before the
      // early return (mirrors the main loop's persistAuditRows; idempotent via
      // business-key dedup) — do not discard Phase-2 work on this branch.
      for (const dc of deliberated) {
        persistAuditRows(
          db,
          {
            findingId: dc.candidate.findingId,
            hitchId: dc.candidate.hitchId,
            harnessSeverity: dc.candidate.severity,
          },
          dc.outcome,
